        ...
        methods: {
            ...
            loadOptions() {
                ...
                Promise.all([...ProjectConf.moduleNames.filter(moduleName => moduleName !== 'login').map(moduleName => new Promise((moduleResolve, moduleReject) => {
                    ...
                    //全部准备完毕
                    if (isEmpty(this.primaryRole)) {
                        alert('您不使用访问本系统的权限');
                        return;
                    }
                    switch (this.primaryRole.value) {
                        case 'tour-speak_admin':
                            this.pageToModule('CourseCenter');
                            break;
                        case 'tour-speak_course-teacher':
                            this.pageToModule('CourseCenter');
                            break;
                        case 'tour-speak_teacher':
                            this.pageToModule('CourseCenter');
                            break;
                        default:
                            alert('角色不正确');
