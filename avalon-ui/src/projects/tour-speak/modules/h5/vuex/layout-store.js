...
const MODULE_LAYOUT = {
    state: {
        ...
        tabKeepAliveCache: [], //选项卡 keep-alive 组件缓存
        driver: new Driver({
            animate: true,//是否开启动画
            opacity: 0.75,//遮罩层透明度
    ...
    actions: {
         ...
         * @param name
         */
        addTabKeepAliveCache(module, name) {
            if (module.state.keepAliveCacheForTabEnabled !== true) return;
            if (module.state.tabKeepAliveCache.indexOf(name) !== -1) return;
            module.state.tabKeepAliveCache.push(name);
        },
        /**
         * 移除选项卡缓存
         * @param module
         * @param name
         */
        removeTabKeepAliveCache(module, name) {
            if (module.state.keepAliveCacheForTabEnabled !== true) return;
            if (module.state.closeTabAndRemoveKeepAliveCacheEnabled !== true) return;
            if (module.state.tabKeepAliveCache.indexOf(name) === -1) return;
            for (let i = 0; i < module.state.tabKeepAliveCache.length; i++) {
                if (name === module.state.tabKeepAliveCache[i]) {
                    module.state.tabKeepAliveCache.splice(i, 1);
                    return;
                }
            }
        },
