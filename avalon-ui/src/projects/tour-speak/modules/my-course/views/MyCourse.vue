<template>
    <div class="common-bg-default">
        <h1 class="tour-layout-right-title">我的课程</h1>
        <!--查询-->
        <div class="tour-search-bar">
            ...
            <div class="common-float-right">
                <el-radio-group v-model="params.classStatus" size="small">
                    <el-radio-button label="">未结束</el-radio-button>
                    <el-radio-button label="CLASS_OVER">已结束</el-radio-button>
                </el-radio-group>
            </div>
        ...
        <div :style="{'height': isEmpty(tableRows) ? '600px' : 'auto'}">
            <we-skeleton v-model="tableSkeleton" animation :paragraph-rows="10"
                ...
                <div v-loading="!tableSkeleton && tableLoading" class="tour-img-main" style="min-height: 600px;">
                    <error-page v-if="isEmpty(tableRows)" :image-width="180" :image-height="180"></error-page>
                    <ul v-else class="tour-img-list">
                        <li v-for="(row, index) in tableRows" @click="routerToMyCourseDetails({courseId: row.id})">
    ...
    export default {
        ...
        methods: {
            ...
            getRows() {
                ...
                this.$Ajax.get(FamSignUrls.get.pageListMySignClass, merge({}, this.params, this.limit))
                    .success(true, data => {
